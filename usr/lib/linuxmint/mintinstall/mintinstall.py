...
class ScreenshotDownloader(threading.Thread):
    ...
    def run(self):
        ...
        # get screenshots from hamonikr community
        try:
            from urllib.request import urlopen
            from bs4 import BeautifulSoup

            hlink = "https://hamonikr.org/board_softwaremngr"
                
            webpage = requests.get(hlink)
            soup = BeautifulSoup(webpage.content,"html.parser")
            maximum = 0
            page_list = soup.findAll("a", {"class": "direction next"})[0]
            pp = str(page_list.attrs['href'])
            ppp = pp.split(';')[0]
            
            maximum = ppp[59:]
            
            i = int(maximum)
            base_url = 'https://hamonikr.org/index.php?mid=board_softwaremngr&page={}'
            
            for page in range(i):
                url = base_url.format(page+1)
                webpage = urlopen(url)
                websource = BeautifulSoup(webpage,'html.parser')
                alist = websource.select_one('#board_list > table > tbody > tr > td.title > a:contains("'+ self.pkginfo.name+'")')
                if alist != '' :   
                    try :
                        hlink = alist.attrs['href']
                        break
                    except : 
                        pass 
        
            webpage2 = requests.get(hlink)
            soup2 = BeautifulSoup(webpage2.content,"html.parser")
            
            cnt=1
        
            read_body = soup2.find('div', {"class": "read_body"})
            img_list2 =  read_body.find_all(['img'])
            
            for image in img_list2:
                image = image.get('src')
                print("cnt : %s\n"% cnt)
                print("img_list2222 : %s\n"% image)
                basename = os.path.basename(image)
                file_name = os.path.splitext(basename)[0]
                print(file_name)

                local_name = os.path.join(SCREENSHOT_DIR, "%s_%s.png" % (self.pkginfo.name, cnt))
                local_thumb = os.path.join(SCREENSHOT_DIR, "thumb_%s_%s.png" % (self.pkginfo.name, cnt))

                self.save_to_file(image, local_name)
                self.save_to_file(image, local_thumb)

                self.application.add_screenshot(self.pkginfo.name, cnt)
...
class Application(Gtk.Application):
    ...
    def __init__(self):
        ...
        print("Hamonikr MintInstall: Detected system architecture: '%s'" % self.arch)
    ...
    def do_command_line(self, command_line, data=None):
        ...
        if num > 1 and args[1] == "list":
            ...
            print("Hamonikr MintInstall: file not found", args[2])
            sys.exit(1)
        elif num > 1:
            print("Hamonikr MintInstall: Unknown arguments", args[1:])
            sys.exit(1)
    ...
    def get_low_res_screen(self):
        ...
        if height < 768:
            print("Hamonikr MintInstall: low resolution detected (%dpx height), limiting window height." % (height))
            return True
    ...
    def create_window(self, starting_page):
        if self.main_window != None:
            print("Hamonikr MintInstall: create_window called, but we already had one!")
            return
        ...
        self.main_window.set_title(_("HamoniKR Software Manager"))
    ...
    def show_package(self, pkginfo, previous_page):
        ...
        # go to hamonikr community 
        community_link = "https://community.linuxmint.com/software/view/%s" % pkginfo.name
        hamonikr_community_link = "https://hamonikr.org/board_softwaremngr"
    
        from urllib.request import urlopen
        from bs4 import BeautifulSoup
        webpage = requests.get(hamonikr_community_link)
        soup = BeautifulSoup(webpage.content,"html.parser")
        maximum = 0
        page_list = soup.findAll("a", {"class": "direction next"})[0]
        pp = str(page_list.attrs['href'])
        ...
        for page in range(i):
            ...
            webpage = urlopen(url)
            websource = BeautifulSoup(webpage,'html.parser')
            alist = websource.select_one('#board_list > table > tbody > tr > td.title > a:contains("'+pkginfo.name+'")')
            
            if alist != '' :   
                try :
                    link = alist.attrs['href']
                    hlink = link.replace("&","&amp;")
                    break
                except : 
                    hlink = hamonikr_community_link
                    pass

        if link !='' :
            webpage2 = requests.get(link)
            soup2 = BeautifulSoup(webpage2.content,"html.parser")
            txt_list = soup2.find('div', {"class": "read_body"}).text
            description = txt_list
        ...
        name = pkginfo.name.lower().find('hamonikr')            
        sum = summary.lower().find('hamonikr')
        des =  description.lower().find('hamonikr')


        if name != -1 :
            self.builder.get_object("label_community") \
            .set_markup(_("Click <a href='%s'>here</a> to add your own review.") % hlink)
        else:
            if sum != -1 :
                self.builder.get_object("label_community") \
                .set_markup(_("Click <a href='%s'>here</a> to add your own review.") % hlink)
